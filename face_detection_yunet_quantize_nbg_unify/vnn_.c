...
#define NET_NORM_TENSOR_NUM     (13)
#define NET_CONST_TENSOR_NUM    (0)
#define NET_VIRTUAL_TENSOR_NUM  (12)
...
vsi_nn_graph_t * vnn_CreateYunetNBG
    (
    ...
    )
{
    ...
    vsi_nn_SetGraphOutputs( graph, NULL, 12 );
 ...
 -----------------------------------------*/
    ...
    /*-----------------------------------------
      ...
      output    - [1, 6400, 1]
                  ...
                  [10, 400, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[0], VSI_NN_OP_NBG, 1, 12, 0);
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    /* @attach_Sigmoid_Sigmoid_85/out0_0:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 1;
    attr.size[1] = 6400;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.003548495704308152;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[0], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Sigmoid_Sigmoid_93/out0_1:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 1;
    attr.size[1] = 1600;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.002753592561930418;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[1], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Sigmoid_Sigmoid_101/out0_2:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 1;
    attr.size[1] = 400;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.003086360404267907;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[2], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Sigmoid_Sigmoid_109/out0_3:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 1;
    attr.size[1] = 6400;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.003912078216671944;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[3], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Sigmoid_Sigmoid_117/out0_4:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 1;
    attr.size[1] = 1600;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.0029951282776892185;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[4], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Sigmoid_Sigmoid_125/out0_5:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 1;
    attr.size[1] = 400;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.002379539655521512;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[5], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Reshape_Reshape_132/out0_6:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 4;
    attr.size[1] = 6400;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.01599474996328354;
    attr.dtype.zero_point = 90;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[6], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Reshape_Reshape_139/out0_7:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 4;
    attr.size[1] = 1600;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.007046951912343502;
    attr.dtype.zero_point = 105;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[7], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Reshape_Reshape_146/out0_8:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 4;
    attr.size[1] = 400;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.012305625714361668;
    attr.dtype.zero_point = 82;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[8], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Reshape_Reshape_153/out0_9:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 10;
    attr.size[1] = 6400;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.030466530472040176;
    attr.dtype.zero_point = 97;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[9], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Reshape_Reshape_160/out0_10:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 10;
    attr.size[1] = 1600;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.005667348857969046;
    attr.dtype.zero_point = 16;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[10], attr, VSI_NN_TYPE_UINT8);

    /* @attach_Reshape_Reshape_167/out0_11:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 10;
    attr.size[1] = 400;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.02915717288851738;
    attr.dtype.zero_point = 91;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[11], attr, VSI_NN_TYPE_UINT8);
    ...
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[12], attr, VSI_NN_TYPE_UINT8);
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    node[0]->input.tensors[0] = norm_tensor[12];
    ...
    node[0]->output.tensors[6] = norm_tensor[6];
    node[0]->output.tensors[7] = norm_tensor[7];
    node[0]->output.tensors[8] = norm_tensor[8];
    node[0]->output.tensors[9] = norm_tensor[9];
    node[0]->output.tensors[10] = norm_tensor[10];
    node[0]->output.tensors[11] = norm_tensor[11];
    ...
    graph->input.tensors[0] = norm_tensor[12];
    ...
    graph->output.tensors[6] = norm_tensor[6];
    graph->output.tensors[7] = norm_tensor[7];
    graph->output.tensors[8] = norm_tensor[8];
    graph->output.tensors[9] = norm_tensor[9];
    graph->output.tensors[10] = norm_tensor[10];
    graph->output.tensors[11] = norm_tensor[11];
...
error:
    ...
    vnn_ReleaseYunetNBG( graph, release_ctx );

    return NULL;
} /* vsi_nn_Create() */

void vnn_ReleaseYunetNBG
    (
    vsi_nn_graph_t * graph,
    vsi_bool release_ctx
    )
