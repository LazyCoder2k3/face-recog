    ...
    )
{
    ...
    return vnn_PostProcess( graph, inputs, input_num );
}
...
for (int times = 0; times < 1; times++) {
    ...
    uint64_t tmsStart, tmsEnd, msVal, usVal;
    tmsStart = get_perf_count();
    printf("Start vnn_PostProcessNeuralNetwork...\n");
    /* Post process output data */
    status = vnn_PostProcessNeuralNetwork( graph, argc, argv );
    TEST_CHECK_STATUS( status, final );
    tmsEnd = get_perf_count();
    msVal = (tmsEnd - tmsStart)/1000000;
    usVal = (tmsEnd - tmsStart)/1000;
    printf("vnn_PostProcessNeuralNetwork: %"VSI_UINT64_SPECIFIER"ms or %"VSI_UINT64_SPECIFIER"us\n", msVal, usVal);
}
