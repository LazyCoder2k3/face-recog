/****************************************************************************
*   Unified Post-Process Source for Yunet and Sface
****************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
const static vsi_nn_postprocess_map_element_t* postprocess_map_sface = NULL;

/*-------------------------------------------
                  Helper Functions
-------------------------------------------*/

static float* vmx_ConvertTensorToFloat32
    (
    ...
    )
{
    ...
    for(i = 0; i < graph->output.num; i++)
    {
        ...
        if (tensor) {
            data = vmx_ConvertTensorToFloat32(graph, tensor);
...
vsi_status vnn_PostProcessSface(vsi_nn_graph_t *graph)
{
    ...
    buffer = vmx_ConvertTensorToFloat32(graph, tensor);
    
    // Original code didn't do anything with buffer, but we should free it to avoid leak
...
std::vector<float> vnn_PostProcessSfaceExtractFeature(vsi_nn_graph_t *graph)
{
    ...
    tensor_data = vmx_ConvertTensorToFloat32(graph, tensor);

    std::vector<float> features(tensor_data, tensor_data + sz);
...
{
    return postprocess_map_sface;
}

uint32_t vnn_GetPostProcessMapCountSface()
{
    if (postprocess_map_sface == NULL)
       return 0;
    else
        return sizeof(postprocess_map_sface) / sizeof(vsi_nn_postprocess_map_element_t);
}
